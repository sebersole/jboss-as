...
public class PersistenceUnitParseProcessor implements DeploymentUnitProcessor {
    ...
    private void handleWarDeployment(DeploymentPhaseContext phaseContext) throws DeploymentUnitProcessingException {
        ...
        if (isWarDeployment(deploymentUnit)) {
            ...
            parse(persistence_xml, listPUHolders, deploymentUnit);
            deploymentRoot.putAttachment(PersistenceUnitMetadataHolder.PERSISTENCE_UNITS, normalize(listPUHolders));
            ...
            for (ResourceRoot resourceRoot : resourceRoots) {
                if (resourceRoot.getRoot().getLowerCaseName().endsWith(".jar")) {
                    ...
                    parse(persistence_xml, listPUHolders, deploymentUnit);
                    resourceRoot.putAttachment(PersistenceUnitMetadataHolder.PERSISTENCE_UNITS, normalize(listPUHolders));
                }
            }
        }
    }
    ...
    private void parse(VirtualFile persistence_xml, List<PersistenceUnitMetadataHolder> listPUHolders, DeploymentUnit deploymentUnit) throws
        ...
        if (persistence_xml.exists() && persistence_xml.isFile()) {
            ...
            try {
                ...
                postParseSteps(persistence_xml, puHolder);
     ...
     * @param puHolder
     */
    private void postParseSteps(VirtualFile persistence_xml, PersistenceUnitMetadataHolder puHolder) {
        ...
        for (PersistenceUnitMetadata pu : puHolder.getPersistenceUnits()) {
            ...
            // set JNDI datasources
            String jtaDataSourceName = pu.getJtaDataSourceName();
            String nonJtaDataSourcename = pu.getNonJtaDataSourceName();

            try {
                if (jtaDataSourceName != null) {
                    pu.setJtaDataSource((DataSource) Util.lookup(jtaDataSourceName, DataSource.class));
                }
            }
            catch( Exception e) {
                throw new RuntimeException("couldn't locate jta DataSource " + jtaDataSourceName, e);
            }

            try {
                if (nonJtaDataSourcename != null) {
                    pu.setNonJtaDataSource((DataSource) Util.lookup(nonJtaDataSourcename, DataSource.class));
                }
            }
            catch( Exception e) {
                throw new RuntimeException("couldn't locate non-jta DataSource " + nonJtaDataSourcename, e);
            }
